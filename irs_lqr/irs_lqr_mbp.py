...
class IrsLqrMbp:
    ...
    def local_descent(self, x_trj, u_trj):
        ...
        if self.x_bounds_abs is not None:
            ...
            x_bounds_abs[0] = self.x_bounds_abs[0]
            x_bounds_abs[1] = self.x_bounds_abs[1]
        if self.u_bounds_abs is not None:
            # u_bounds_abs are used to establish input constraints.
            u_bounds_abs = np.zeros((2, self.T, self.dim_u))
            u_bounds_abs[0] = self.u_bounds_abs[0]
            u_bounds_abs[1] = self.u_bounds_abs[1]
