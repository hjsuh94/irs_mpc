...
from irs_lqr.cem_mbp_position import CrossEntropyMethodMbpPosition
...
qa_l_knots[0] = [-np.pi / 2 + 0.5, -np.pi / 2 + 0.5, 0.0, 0.0]
q_robot_l_traj = PiecewisePolynomial.ZeroOrderHold(
    [0, T * h], qa_l_knots.T)

qa_r_knots = np.zeros((2, nq_a))
qa_r_knots[0] = [np.pi / 2 - 0.5, np.pi / 2 - 0.5, 0.0, 0.0]
...
params.Q_dict = {
    idx_u: np.array([10, 1, 10, 0.0, 0.0, 0.0]),
...
xd_dict = {idx_u: q_u0 + np.array([0.0, -0.2, -np.pi/4, 0, 0, 0]),
...
params.initial_std = 0.1
params.publish_every_iteration = True

irs_lqr_q = CrossEntropyMethodMbpPosition(mbp_dynamics, params)
...
np.savetxt("examples/planar_hand/analysis/planar_hand_spin_second_cem.csv",
